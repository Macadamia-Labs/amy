...
interface CooperContextType {
  ...
  setRequirements: (
    requirements:
      ...
      | ((prev: SimulationRequirement[]) => SimulationRequirement[])
  ) => void

  // Steps
  simulationSteps: SimulationStep[]
  setSimulationSteps: (
    steps: SimulationStep[] | ((prev: SimulationStep[]) => SimulationStep[])
  ) => void
  ...
  setToolInvocations: (
    invocations:
      | ToolInvocation[]
      | ((prev: ToolInvocation[]) => ToolInvocation[])
  ) => void
...
function areReqDependenciesMet(
  ...
  if (
    !step.requirementDependencies ||
    step.requirementDependencies.length === 0
  )
    return true
...
function canGenerateCode(
  ...
  return (
    areStepDependenciesMet(step, steps) &&
    areReqDependenciesMet(step, requirements)
  )
}
...
export function CooperProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const needsCodeGeneration = simulationSteps.some(
      step =>
        !step.codeGenerated &&
        canGenerateCode(step, simulationSteps, requirements)
    )
  ...
  return (
    <CooperContext.Provider value={value}>{children}</CooperContext.Provider>
  )
}

/**
 * Hook to use the Cooper context
 */
export function useCooper() {
  const context = useContext(CooperContext)
  if (!context) {
    throw new Error('useCooper must be used within an CooperProvider')
  }
  return context
}
