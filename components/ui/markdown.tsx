    ...
    components={{
      ...
      table: ({ children, ...props }) => (
        <div className="w-full overflow-auto my-4">
          <table className="w-full border-collapse rounded-lg border border-border shadow-sm overflow-hidden" {...props}>
            {children}
          </table>
        </div>
      ),
      ...
      th: ({ children, ...props }) => (
        <th className="px-4 py-3 text-left font-semibold bg-muted/50" {...props}>
          {children}
        </th>
      ),
      td: ({ children, ...props }) => (
